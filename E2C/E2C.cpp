...
int main()
{
	//populate clue table with clue data
	//Clue pieces
	ClueTable[50].IsClue = true;
	ClueTable[61].IsClue = true;
	ClueTable[151].IsClue = true;
	ClueTable[226].IsClue = true;
	ClueTable[237].IsClue = true;

	//populate lookaheads
	ClueTable[34].Top = 17;
	ClueTable[34].Check = true;

	ClueTable[45].Top = 17;
	ClueTable[45].Check = true;

	ClueTable[135].Top = 15;
	ClueTable[135].Check = true;

	ClueTable[210].Top = 7;
	ClueTable[210].Check = true;

	ClueTable[221].Top = 12;
	ClueTable[221].Check = true;

	ClueTable[49].Left = 13;
	ClueTable[49].Check = true;

	ClueTable[60].Left = 7;
	ClueTable[60].Check = true;

	ClueTable[150].Left = 20;
	ClueTable[150].Check = true;

	ClueTable[225].Left = 20;
	ClueTable[225].Check = true;

	ClueTable[236].Left = 10;
	ClueTable[236].Check = true;
...
//void savetables()
//{
//	std::string xfile = "xpieces.txt";
//	std::string lfile = "lookup.txt";
//
//	std::ofstream xout(xfile);
//	if (xout.is_open())
//	{
//		for (int x = 0; x < 4096; x++)
//		{
//			for (int j = 0; j < MAX_LOOKUP; j++)
//			{
//
//			}
//		}
...
void solve2()
{
	...
	do
	{
		...
		if (ClueTable[currentposition].IsClue)
			currentposition++;
		else
		{
			...
			while (index < lookupcount[piecekey] && !abandon)
			{
				...
				{
					if (ClueTable[currentposition].Check)
					{
						if (ClueTable[currentposition].Left != 0 && (((xpiece[actualpiece] >> 12) & 0x001F) != ClueTable[currentposition].Left))
						{
							index++;
							continue;
						}
						if (ClueTable[currentposition].Top != 0 && (((xpiece[actualpiece] >> 17) & 0x001F) != ClueTable[currentposition].Top))
						{
							index++;
							continue;
						}
			...
			if (index >= lookupcount[piecekey])
			{
				...
				else
				{
					...
					while (ClueTable[currentposition].IsClue)
						currentposition--;
